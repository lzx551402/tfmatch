...
class VisualContext(Network):
    ...
    def _mlp(self, inputs, mlp, last_relu=True, reuse=False, name='conv_%d'):
        new_points = tf.expand_dims(inputs, axis=2)
        for i, num_out_channel in enumerate(mlp):
            if (i == len(mlp) - 1) and not last_relu:
                new_points = self.conv_bn(new_points, 1, num_out_channel,
    ...
    def setup(self):
        ...
        grid_pts = tf.tile(tf.expand_dims(
            tf.stack((xv, yv), -1), 0), (batch_size, 1, 1, 1))
        ...
        inter_vis_context_feat = self._interpolate(
            kpt_ncoords, grid_pts, trans_vis_context_feat)
        fused_feat = tf.concat((inter_vis_context_feat, local_feat), axis=-1)

        out_vis_context = self._mlp(
            fused_feat, [512, 256, 128], name='fuse_photo_context_%d')
