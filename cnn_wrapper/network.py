...
from .homo_utils import solve_DLT
...
class Network(object):
    ...
    def __init__(self, inputs, is_training,
                 dropout_rate=0, seed=None, epsilon=1e-5, reuse=False, fcn=True, regularize=True,
                 **kwargs):
    ...
    @layer
    def group_normalization(self, input_tensor, name,
                            channel_wise=True, group=32, group_channel=8,
        ...
        with tf.variable_scope(name, reuse=self.reuse or reuse):
            if scale:
                gamma = tf.get_variable('gamma', [C], dtype=tf.float32,
                                        initializer=tf.ones_initializer())
            else:
                gamma = tf.constant(1.0, shape=[C])
            if center:
                beta = tf.get_variable('beta', [C], dtype=tf.float32,
    ...
    def conv_gn(self,
        ...
        conv_gn = self.group_normalization(conv, name + '/gn',
                                           channel_wise=channel_wise, group=group, group_channel=group_channel,
    ...
    @layer
    def sigmoid(self, input_tensor, name=None):
        return tf.math.sigmoid(input_tensor, name=name)
    ...
    def deform_conv(self,
        ...
        # some length
        if self.training:
            batch_size, in_h, in_w = [inputs.get_shape()[i].value for i in range(0, 3)]
        else:
            batch_size, in_h, in_w = [tf.shape(inputs)[i] for i in range(0, 3)]
        ...
        with tf.compat.v1.variable_scope('deform_param', reuse=reuse):
            if deform_type == 'a':
                ...
                if self.training:
                    offset = tf.reshape(offset, [batch_size, comb_A.get_shape()[1].value,
                ...
                offset = tf.stack([offset[:, :, :, :, 1], offset[:, :, :, :, 0]], axis=-1)
                if self.training:
                    offset = tf.reshape(offset, [batch_size, H_mat.get_shape()[1].value,
    ...
    def deconv_gn(self,
        ...
        deconv_gn = self.group_normalization(deconv, name + '/gn',
                                             channel_wise=channel_wise, group=group, group_channel=group_channel,
                                             center=center, scale=scale, relu=relu, reuse=reuse)
        return deconv_gn
    ...
    @layer
    def batch_normalization(self, input_tensor, name, axis=-1,
                            center=False, scale=False, relu=False, reuse=False):
        """Batch normalization."""
    ...
    @layer
    def l2norm(self, input_tensor, name=None, axis=-1):
        return tf.nn.l2_normalize(input_tensor, axis=axis, name=name)

    @layer
    def square(self, input_tensor, name):
        return tf.square(input_tensor, name=name)
